...
class KademliaClient:
    def __get_conn_string(self, container_info: tuple):
        return f"{"127.0.0.1" if os.getenv("LOCAL") == "TRUE" else container_info[0]}:{container_info[2] if os.getenv("LOCAL") == "TRUE" else container_info[1]}"

    async def PING(self, container_info: tuple):
        async with grpc.aio.insecure_channel(self.__get_conn_string(container_info)) as channel:
